...
app.logUserOut = function(redirectUser){
  ...
  app.client.request(undefined,'api/tokens','DELETE',queryStringObject,undefined,function(statusCode,responsePayload){
    ...
    if(redirectUser){
      window.localStorage.setItem("currentOrderId", "")
  ...
  if(email){
    ...
    app.client.request(undefined, 'api/users', 'GET', queryStringObject, undefined,function(statusCode,responsePayload){
      if(statusCode == 200){
        ...
        app.client.request(undefined, 'api/menu', 'GET', newQueryStringObject, undefined,function(statusCode,responsePayload){
          if(statusCode == 200){
            var index = 0;
            responsePayload.forEach(function(item){
              var table = document.getElementById("menuTable");
              var tr = table.insertRow(-1);
              tr.classList.add('orderRow' + index);
              var td0 = tr.insertCell(0);
              var td1 = tr.insertCell(1);
              var td2 = tr.insertCell(2);
              td0.innerHTML = item.name;
              td1.innerHTML = (item.price/100).toFixed(2)+' $';
              td2.innerHTML = '<input type="number" id="amount' + index + '" value="0" min="0" style="width: 50px" />';
              index += 1;
            });
          } else {
            // If the request comes back as something other than 200, log the user our (on the assumption that the api is temporarily down or the users token is bad)
  ...
  if(email){
    ...
    for (var i = 1; i < rows.length; i += 1) {
      ...
      for (var j = 0; j < cells.length; j += 1) {
        if (j == 0) {
          order[i - 1].name = cells[j].innerHTML;
        }
        if (j == 1) {
          order[i - 1].price = cells[j].innerHTML.replace(' $', '') * 100;
        }
        if (j == 2) {
          var input = document.getElementById(`amount${i - 1}`);
          order[i - 1].quantity = parseInt(input.value);
        }
      }
    }
    ...
    app.client.request(undefined, 'api/orders', 'POST', undefined, requestPayload, function(statusCode,responsePayload){
      if(statusCode == 200){
        window.localStorage.setItem("currentOrderId", responsePayload.id);
...
app.loadReviewPage = function(){
  var orderId = window.localStorage.getItem("currentOrderId");
  var email = typeof(app.config.sessionToken.email) == 'string' ? app.config.sessionToken.email : false;
  ...
  if(email){
    ...
    app.client.request(undefined,'api/orders','GET',queryStringObject,undefined,function(statusCode,responsePayload){
      if(statusCode == 200){
        var subHeader = document.getElementById('orderId');
        var totalPrice = document.getElementById('totalPrice');
        subHeader.innerHTML = "Order Number " + responsePayload.id;
        totalPrice.innerHTML = "Total Price " + (responsePayload.totalPrice/100).toFixed(2) + " $";

        responsePayload.order.forEach(function(item){
          ...
          var td2 = tr.insertCell(2);
          td0.innerHTML = item.name;
          td1.innerHTML = (item.price/100*item.quantity).toFixed(2)+' $';
...
app.checkout = function(){
  var orderId = document.getElementById('orderId').innerHTML.replace("Order Number ", "");
  ...
  if(email && orderId && number && exp_month && exp_year && cvc){
    ...
    app.client.request(undefined,'api/checkout','POST',undefined,requestPayload,function(newStatusCode,newResponsePayload){
      if(newStatusCode == 200){
        window.location = '/dashboard';
      } else {
        window.location = '/dashboard';
